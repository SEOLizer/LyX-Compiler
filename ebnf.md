# Lyx 1.0.14L — Canonical EBNF Grammar

> Stand 2026-08-09, gegen lyxc 1.0.14L geprueft. Die Keyword-Liste in
...
| Attribute ohne Nachweis | 4 | `@integrity`, `@flight_crit`, `@dal` und `@critical` werden geparst, in ihrer Argumentform geprueft und am Knoten vermerkt -- der Compiler weist die Zusicherung aber **nicht** nach. Keine TMR-Verifikation, keine FPU-Traps. `@stack_limit` (seit 1.0.14K, #1138) und `@wcet` (seit 1.0.14L, #1139) sind NICHT mehr darunter. Sie sind seit #1099 nicht mehr stumm: jedes Vorkommen meldet den fehlenden Nachweis. Wer die Annotation setzt, bekommt sie also als Vermerk, nicht als Beweis. Unbekannte Attributnamen und falsche Argumentformen werden abgewiesen. |
