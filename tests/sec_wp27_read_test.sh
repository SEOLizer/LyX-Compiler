...
if grep -q "if uiSz < 0" "$LYXC"; then _pass 3; else _fail 3 "--unit-info EINTR-Guard fehlt"; fi

# 4: ELF section-header read-Guard
if grep -q "!= shdrsSize" "$LYXC"; then _pass 4; else _fail 4 "ELF shdrs read-Guard fehlt"; fi

# 5: ELF shstrtab read-Guard
if grep -q "!= strsSize" "$LYXC"; then _pass 5; else _fail 5 "ELF strtab read-Guard fehlt"; fi

# 6: License-Payload read-Guard (16 Bytes)
if grep -q "16) != 16" "$LYXC"; then _pass 6; else _fail 6 "License-Payload read-Guard fehlt"; fi
