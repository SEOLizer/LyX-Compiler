...
| `std/hash.lyx` SHA-256 | **Nur 1 Block (≤55 Bytes)** | Multi-Block nötig (WP-PQC-02) |
| `std/hash.lyx` SHA-3/SHAKE | **Fake-Stubs** (XOR mit Konstanten) | Echter Keccak-1600 (WP-PQC-01) |
...
### WP-PQC-02 — SHA-256 Multi-Block (`std/hash.lyx` Erweiterung)

**Ziel:** Die bestehende `HashSHA256` in `hash.lyx` verarbeitet nur Eingaben ≤ 55 Bytes (ein 512-Bit-Block). Für SLH-DSA und HMAC-SHA256 sind beliebig lange Eingaben nötig.

**Datei:** `std/hash.lyx` (bestehende Funktion `HashSHA256` ersetzen)

**Fehlender Teil:** Die Nachrichtenplanung (Message Schedule W[0..63]) wird aktuell nur für Block 0 berechnet. Es fehlt die äußere Loop über alle Blöcke:
```
for block in 0..(len / 64):
    W[0..15] = message_block[block]
...
pub type SHA256State = struct {
    h:      int64;   // Pointer auf 8 × int64 (256-Bit Zustand)
    buf:    int64;   // Pointer auf 64-Byte Puffer
    bufLen: int64;   // Bytes im Puffer
    total:  int64;   // Gesamtbytes verarbeitet
};

pub fn SHA256Init(state: int64): void;
pub fn SHA256Update(state: int64, data: int64, len: int64): void;
pub fn SHA256Final(state: int64, out: int64): void;   // out = 32 Bytes

// Convenience (wie bisher, aber jetzt korrekt für beliebige Länge):
pub fn HashSHA256Bytes(data: int64, len: int64, out: int64): void;
```

**Akzeptanzkriterium:** NIST-Testvektoren für Eingaben von 0 bis 10.000 Bytes passen.
