...
begin
  ...
  // StrLength(pchar) -> int64
  s := TSymbol.Create('StrLength');
  s.Kind := symFunc;
  s.DeclType := atInt64;
  s.ParamCount := 1;
  SetLength(s.ParamTypes, 1);
    ...
    nkVarDecl:
      begin
        ...
        begin
          ...
          // Special case: dynamic array (atDynArray) with array literal or atDynArray init
          else if (vd.DeclType = atDynArray) and
                  ((vtype = atDynArray) or (vtype = atUnresolved) or
                   (Assigned(vd.InitExpr) and (vd.InitExpr is TAstArrayLit))) then
            vtype := vd.DeclType  // Accept dynamic array type with any array literal
          else
