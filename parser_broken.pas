...
begin
  ...
  begin
    ...
    // Expect 'Array'
    if not Check(tkArray) then
    begin
      FDiag.Error('expected ''Array'' after ''parallel''', FCurTok.Span);
      Exit(TAstIntLit.Create(0, span));
    end;
    Advance; // consume 'Array'
    
    // Expect <Type>
    if not Accept(tkLt) then
    begin
      FDiag.Error('expected ''<'' for generic type parameter', FCurTok.Span);
      Exit(TAstIntLit.Create(0, span));
    end;
    ...
    if not Check(tkIdent) then
    begin
      FDiag.Error('expected element type in ParallelArray<T>', FCurTok.Span);
      Exit(TAstIntLit.Create(0, span));
    end;
    name := FCurTok.Value;  // element type name (e.g., "Int", "Int64", "Float")
    span := FCurTok.Span;
    Advance;
    
    if not Accept(tkGt) then
    begin
      FDiag.Error('expected ''>'' to close generic type parameter', FCurTok.Span);
      Exit(TAstIntLit.Create(0, span));
    end;
    
    // Expect (size)
    if not Accept(tkLParen) then
    begin
      FDiag.Error('expected ''('' for ParallelArray size', FCurTok.Span);
      Exit(TAstIntLit.Create(0, span));
    end;
    ...
    // Map common type names to AurumType
    if (name = 'Int') or (name = 'Int64') then
      Result := TAstSIMDNew.Create(atInt64, name, e, span)
    else if name = 'Int32' then
      Result := TAstSIMDNew.Create(atInt32, name, e, span)
    else if name = 'Int16' then
      Result := TAstSIMDNew.Create(atInt16, name, e, span)
    else if name = 'Int8' then
      Result := TAstSIMDNew.Create(atInt8, name, e, span)
    else if (name = 'UInt') or (name = 'UInt64') then
      Result := TAstSIMDNew.Create(atUInt64, name, e, span)
    else if name = 'UInt32' then
      Result := TAstSIMDNew.Create(atUInt32, name, e, span)
    else if name = 'UInt16' then
      Result := TAstSIMDNew.Create(atUInt16, name, e, span)
    else if name = 'UInt8' then
      Result := TAstSIMDNew.Create(atUInt8, name, e, span)
    else if (name = 'Float') or (name = 'Float64') or (name = 'F64') then
      Result := TAstSIMDNew.Create(atF64, name, e, span)
    else if (name = 'Float32') or (name = 'F32') then
      Result := TAstSIMDNew.Create(atF32, name, e, span)
    else
    begin
      // Try to resolve as named type
      Result := TAstSIMDNew.Create(atUnresolved, name, e, span);
    end;
    Exit;
  end;
...
begin
  ...
  begin
    // Check for ParallelArray<T> syntax
    if Accept(tkParallel) then
    begin
      // Expect 'Array'
      if not Check(tkArray) then
      begin
        // Not ParallelArray, treat as identifier
        s := 'parallel';
      ...
      begin
        Advance; // consume 'Array'
        // Parse generic type parameter: <Type>
        if not Accept(tkLt) then
        begin
          // No generic parameter, treat as identifier
          s := 'parallelarray';
          Result := StrToAurumType(s);
          if Result = atUnresolved then
            typeName := s;
        ...
        begin
          ...
          begin
            ...
            if (s = 'Int') or (s = 'Int64') then
              Result := atParallelArray
            else if s = 'Int32' then
              Result := atParallelArray
            else if s = 'Int16' then
              Result := atParallelArray
            else if s = 'Int8' then
              Result := atParallelArray
            else if (s = 'UInt') or (s = 'UInt64') then
              Result := atParallelArray
              ...
              Result := atParallelArray;
            typeName := 'ParallelArray<' + s + '>';
          end
          else
          begin
            Result := atParallelArray;
            typeName := 'ParallelArray';
          end;
          
          if not Accept(tkGt) then
          begin
            // Error in parsing, but continue
          end;
        end;
      end;
    end
    else if Accept(tkArray) then
    begin
