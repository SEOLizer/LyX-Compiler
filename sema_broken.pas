...
function IsValidSIMDElementType(t: TAurumType): Boolean;
begin
  // ParallelArray supports integer and float types
  Result := IsIntegerType(t) or (t in [atF32, atF64]);
end;

function GetSIMDKindForType(t: TAurumType): TSIMDKind;
begin
  case t of
    atInt8, atUInt8:   Result := simdI8;
    atInt16, atUInt16: Result := simdI16;
    atInt32, atUInt32: Result := simdI32;
    atInt64, atUInt64: Result := simdI64;
    atF32:             Result := simdF32;
    atF64:             Result := simdF64;
  else
    Result := simdNone;
  end;
end;
...
begin
  ...
  case expr.Kind of
    ...
    nkSIMDNew:
      begin
        // ParallelArray<T>(size) - SIMD-optimized array
        CheckExpr(TAstSIMDNew(expr).Size);
        // Element type should be a valid scalar type
        if not IsValidSIMDElementType(TAstSIMDNew(expr).ElementType) then
        begin
          FDiag.Error('ParallelArray requires a valid scalar element type (int/float), got: ' + 
            TAstSIMDNew(expr).ElementTypeName, expr.Span);
          Result := atUnresolved;
        end
        else
        begin
          // Set resolved type to ParallelArray
          Result := atParallelArray;
          // Set SIMD kind based on element type
          TAstSIMDNew(expr).SIMDKind := GetSIMDKindForType(TAstSIMDNew(expr).ElementType);
        end;
      end;
    nkSIMDBinOp:
      begin
        // SIMD binary operation: element-wise on vectors
        lt := CheckExpr(TAstSIMDBinOp(expr).Left);
        rt := CheckExpr(TAstSIMDBinOp(expr).Right);
        // Both operands should be ParallelArray
        if (lt <> atParallelArray) or (rt <> atParallelArray) then
        begin
          FDiag.Error('SIMD binary operation requires ParallelArray operands', expr.Span);
          Result := atUnresolved;
        end
        else
        begin
          // Element-wise operation returns ParallelArray
          Result := atParallelArray;
        end;
      end;
    nkSIMDUnaryOp:
      begin
        // SIMD unary operation: element-wise negation
        lt := CheckExpr(TAstSIMDUnaryOp(expr).Operand);
        if lt <> atParallelArray then
        begin
          FDiag.Error('SIMD unary operation requires ParallelArray operand', expr.Span);
          Result := atUnresolved;
        end
        else
        begin
    ...
    nkSIMDIndexAccess:
      begin
        // SIMD index access: vec[index] - returns scalar element
        lt := CheckExpr(TAstSIMDIndexAccess(expr).Obj);
        CheckExpr(TAstSIMDIndexAccess(expr).Index);
        if lt <> atParallelArray then
        begin
          FDiag.Error('SIMD index access requires ParallelArray object', expr.Span);
          Result := atUnresolved;
        end
        else
        begin
          // Index access on ParallelArray returns scalar (element type)
          // For now, return int64 as default element type
          Result := atInt64;
        end;
      end;
  else
