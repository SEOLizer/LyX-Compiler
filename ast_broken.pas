...
type
  ...
  TAurumType = (
    ...
    // SIMD Types (v0.2.0)
    atParallelArray,   // parallel/SIMD array type
    atSIMDVec8,        // 8-element SIMD vector (e.g., for int8 with AVX-512)
    atSIMDVec16,      // 16-element SIMD vector (e.g., for int32 with AVX)
    atSIMDVec32,      // 32-element SIMD vector (e.g., for int16 with AVX-512)
    atSIMDVec64       // 64-element SIMD vector (e.g., for int8 with AVX-512)
  );
  ...
  { SIMD-Vektor-Typ-Information }
  TSIMDKind = (
    simdNone,
    simdI8,    // 8-bit integer vectors
    simdI16,   // 16-bit integer vectors
    simdI32,   // 32-bit integer vectors
    simdI64,   // 64-bit integer vectors
    simdF32,   // 32-bit float vectors
    simdF64    // 64-bit float vectors
  );

  { ParallelArray<T>(size) - SIMD-optimiertes Array }
  TAstSIMDNew = class(TAstExpr)
  private
    FElementType: TAurumType;      // Element type (e.g., atInt64)
    FElementTypeName: string;      // Type name for resolution
    FSize: TAstExpr;              // Size expression
    FSIMDKind: TSIMDKind;         // SIMD vector kind
  public
    constructor Create(aElementType: TAurumType; const aElementTypeName: string;
      aSize: TAstExpr; aSpan: TSourceSpan);
    destructor Destroy; override;
    property ElementType: TAurumType read FElementType;
    property ElementTypeName: string read FElementTypeName;
    property Size: TAstExpr read FSize;
    property SIMDKind: TSIMDKind read FSIMDKind write FSIMDKind;
  end;

  { SIMD Binary Operation: a + b (element-wise auf SIMD-Vektoren) }
  ...
  { SIMD Unary Operation: -x, !flag (element-wise) }
...
begin
  case t of
    ...
    // SIMD Types
    atParallelArray: Result := 'ParallelArray';
    atSIMDVec8:    Result := 'SIMDVec8';
    atSIMDVec16:   Result := 'SIMDVec16';
    atSIMDVec32:   Result := 'SIMDVec32';
    atSIMDVec64:   Result := 'SIMDVec64';
  else
    Result := '<unknown>';
  end;
end;
...
begin
  case s of
    ...
    // SIMD Types
    'ParallelArray': Result := atParallelArray;
    'simd':    Result := atParallelArray;  // alias
  else
    Result := atUnresolved;
  end;
end;
...
constructor TAstSIMDNew.Create(aElementType: TAurumType; const aElementTypeName: string;
  aSize: TAstExpr; aSpan: TSourceSpan);
begin
  inherited Create(nkSIMDNew, aSpan);
  FElementType := aElementType;
  FElementTypeName := aElementTypeName;
  FSize := aSize;
  FSIMDKind := simdNone;
end;
...
constructor TAstSIMDBinOp.Create(aOp: TTokenKind; aLeft, aRight: TAstExpr;
  aSpan: TSourceSpan);
begin
...
constructor TAstSIMDUnaryOp.Create(aOp: TTokenKind; aOperand: TAstExpr;
  aSpan: TSourceSpan);
begin
...
constructor TAstSIMDIndexAccess.Create(aObj: TAstExpr; aIndex: TAstExpr; aSpan: TSourceSpan);
begin
