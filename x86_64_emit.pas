...
uses
 ...
 type
  ...
  private
    ...
    // VMT labels (class name -> data position)
    FVMTLabels: array of TLabelPos;
    // VMT LEA positions for patching (VMT index -> code position)
...
procedure TX86_64Emitter.EmitFromIR(module: TIRModule);
  ...
  skipGrowPos, capOkPos, skipMunmapPos, skipFreePos: Integer;
...
begin
  ...
  begin
    ...
    // Only emit VMT if class has virtual methods (use the property, not the private field)
    ...
    begin
      ...
      begin
        ...
        // For now, emit placeholder - will be patched with actual method address
        // The actual patching will happen when we know the method addresses
        FData.WriteU64LE(0);
        Inc(totalDataOffset, 8);
      end;
    end;
      ...
      case instr.Op of
        ...
        irLoadGlobalAddr:
          begin
            ...
            // Special case: VMT labels (start with _vmt_)
            if Copy(instr.ImmStr, 1, 5) = '_vmt_' then
            begin
              // VMT label - look it up in FVMTLabels
              vmtLabelIdx := -1;
              ...
              if vmtLabelIdx >= 0 then
              begin
                // Found VMT - emit lea to load its address
                // lea rax, [rip+disp32]
                leaPos := FCode.Size;
                EmitU8(FCode, $48); EmitU8(FCode, $8D); EmitU8(FCode, $05); EmitU32(FCode, 0);
                // Record for patching with VMT position
                SetLength(FVMTLeaPositions, Length(FVMTLeaPositions) + 1);
                FVMTLeaPositions[High(FVMTLeaPositions)].VMTIndex := vmtLabelIdx;
                FVMTLeaPositions[High(FVMTLeaPositions)].CodePos := leaPos;
        ...
        irDynArrayPush:
          begin
            ...
            //
            // Algorithm:
            //   if len >= cap then grow(cap = max(8, cap*2), realloc via mmap+copy+munmap)
            //   ptr[len] := value
            //   len := len + 1

            // Load len, cap, ptr
            WriteMovRegMem(FCode, RCX, RBP, SlotOffset(instr.Src1 + 1)); // RCX = len
            WriteMovRegMem(FCode, RDX, RBP, SlotOffset(instr.Src1 + 2)); // RDX = cap
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(instr.Src1));     // RSI = ptr

            // Compare len >= cap
            // cmp rcx, rdx
            EmitU8(FCode, $48); EmitU8(FCode, $39); EmitU8(FCode, $D1); // cmp rcx, rdx
            // jb skip_grow (len < cap => no grow needed)
            skipGrowPos := FCode.Size;
            EmitU8(FCode, $0F); EmitU8(FCode, $82); EmitU32(FCode, 0); // jb rel32
            ...
            WriteMovRegReg(FCode, R12, RCX);  // R12 = len
            WriteMovRegMem(FCode, R13, RBP, SlotOffset(localCnt + instr.Src2)); // R13 = value temp
            WriteMovRegReg(FCode, R14, RSI);  // R14 = old ptr
            WriteMovRegReg(FCode, R15, RDX);  // R15 = old cap

            // new_cap = cap * 2; if new_cap < 8 then new_cap = 8
            WriteMovRegReg(FCode, RAX, R15);
            // shl rax, 1 (cap * 2)
            EmitU8(FCode, $48); EmitU8(FCode, $D1); EmitU8(FCode, $E0);
            // cmp rax, 8
            EmitU8(FCode, $48); EmitU8(FCode, $83); EmitU8(FCode, $F8); EmitU8(FCode, 8);
            // jge cap_ok
            capOkPos := FCode.Size;
            EmitU8(FCode, $0F); EmitU8(FCode, $8D); EmitU32(FCode, 0); // jge rel32
            WriteMovRegImm64(FCode, RAX, 8);
            k := FCode.Size;
            FCode.PatchU32LE(capOkPos + 2, Cardinal(k - capOkPos - 6));

            // Save new_cap on stack
            EmitU8(FCode, $50); // push rax

            // new_size = new_cap * 8 (bytes)
            // shl rax, 3
            EmitU8(FCode, $48); EmitU8(FCode, $C1); EmitU8(FCode, $E0); EmitU8(FCode, 3);

            // mmap(0, new_size, PROT_READ|PROT_WRITE=3, MAP_PRIVATE|MAP_ANON=34, -1, 0)
            WriteMovRegReg(FCode, RSI, RAX);   // RSI = size
            WriteMovRegImm64(FCode, RDI, 0);   // addr = 0
            WriteMovRegImm64(FCode, RDX, 3);   // prot = 3
            WriteMovRegImm64(FCode, R10, 34);  // flags = 34
            WriteMovRegImm64(FCode, R8, High(UInt64)); // fd = -1
            ...
            // Save new_ptr in RBX
            WriteMovRegReg(FCode, RBX, RAX);

            // Copy old data: memcpy(new_ptr, old_ptr, len * 8)
            // RDI = dest (new ptr), RSI = src (old ptr), RCX = count (len)
            WriteMovRegReg(FCode, RDI, RBX);   // dest = new ptr
            WriteMovRegReg(FCode, RSI, R14);   // src = old ptr
            WriteMovRegReg(FCode, RCX, R12);   // count = len
            // rep movsq
            EmitU8(FCode, $F3); EmitU8(FCode, $48); EmitU8(FCode, $A5);

            // munmap old ptr if old_cap > 0
            // Check R15 (old_cap) > 0
            EmitU8(FCode, $4D); EmitU8(FCode, $85); EmitU8(FCode, $FF); // test r15, r15
            skipMunmapPos := FCode.Size;
            EmitU8(FCode, $0F); EmitU8(FCode, $84); EmitU32(FCode, 0); // je skip_munmap

            // munmap(old_ptr, old_cap * 8)
            WriteMovRegReg(FCode, RDI, R14);   // RDI = old ptr
            WriteMovRegReg(FCode, RSI, R15);
            EmitU8(FCode, $48); EmitU8(FCode, $C1); EmitU8(FCode, $E6); EmitU8(FCode, 3); // shl rsi, 3
            WriteMovRegImm64(FCode, RAX, 11);  // sys_munmap
            WriteSyscall(FCode);

            k := FCode.Size;
            FCode.PatchU32LE(skipMunmapPos + 2, Cardinal(k - skipMunmapPos - 6));

            // pop new_cap into RDX
            EmitU8(FCode, $5A); // pop rdx

            // Store new values: ptr = RBX (new_ptr), cap = RDX
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1), RBX);      // ptr = new_ptr
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1 + 2), RDX);  // cap = new_cap

            // Restore len (R12) and value (R13) and ptr (R14) and cap (R15)
            WriteMovRegReg(FCode, RCX, R12);  // RCX = len
            // RSI = new ptr from RBX
            WriteMovRegReg(FCode, RSI, RBX);
            ...
            // Load value into RAX from memory
            WriteMovRegMem(FCode, RAX, RBP, SlotOffset(localCnt + instr.Src2)); // RAX = value

            // Jump target for skip_grow
            k := FCode.Size;
            FCode.PatchU32LE(skipGrowPos + 2, Cardinal(k - skipGrowPos - 6));

            // If we didn't grow, RSI=ptr already loaded, RCX=len already loaded
            // Load value into RAX
            WriteMovRegMem(FCode, RAX, RBP, SlotOffset(localCnt + instr.Src2)); // RAX = value
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(instr.Src1)); // RSI = ptr (reload)
            WriteMovRegMem(FCode, RCX, RBP, SlotOffset(instr.Src1 + 1)); // RCX = len

            // Store value at ptr[len]: mov [rsi + rcx*8], rax
            EmitU8(FCode, $48); EmitU8(FCode, $89); EmitU8(FCode, $04); EmitU8(FCode, $CE);

            // len := len + 1
            WriteIncReg(FCode, RCX);
            // Store new len
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1 + 1), RCX);
          end;
        ...
        irDynArrayPop:
          begin
            ...
            // Load ptr[len] -> RAX: mov rax, [rsi + rcx*8]
            EmitU8(FCode, $48); EmitU8(FCode, $8B); EmitU8(FCode, $04); EmitU8(FCode, $CE);
        ...
        irDynArrayLen:
          begin
            ...
            // Load len into RAX
            WriteMovRegMem(FCode, RAX, RBP, SlotOffset(instr.Src1 + 1)); // RAX = len
        ...
        irDynArrayFree:
          begin
            ...
            //
            // Algorithm:
            //   if ptr != 0: munmap(ptr, cap * 8)
            //   ptr := 0; len := 0; cap := 0

            // Load ptr
            WriteMovRegMem(FCode, RAX, RBP, SlotOffset(instr.Src1)); // RAX = ptr

            // Check if ptr != 0
            EmitU8(FCode, $48); EmitU8(FCode, $83); EmitU8(FCode, $F8); EmitU8(FCode, 0); // cmp rax, 0
            skipFreePos := FCode.Size;
            EmitU8(FCode, $0F); EmitU8(FCode, $84); EmitU32(FCode, 0); // je skip_free

            // munmap(ptr, cap * 8)
            WriteMovRegMem(FCode, RDI, RBP, SlotOffset(instr.Src1)); // RDI = ptr
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(instr.Src1 + 2)); // RSI = cap
            EmitU8(FCode, $48); EmitU8(FCode, $C1); EmitU8(FCode, $E6); EmitU8(FCode, 3); // shl rsi, 3 (cap * 8)
            WriteMovRegImm64(FCode, RAX, 11); // sys_munmap
            WriteSyscall(FCode);

            // Patch skip_free position
            k := FCode.Size;
            FCode.PatchU32LE(skipFreePos + 2, Cardinal(k - skipFreePos - 6));

            // Reset fat-pointer: ptr=0, len=0, cap=0
            WriteMovRegImm64(FCode, RAX, 0);
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1), RAX);     // ptr = 0
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1 + 1), RAX); // len = 0
            WriteMovMemReg(FCode, RBP, SlotOffset(instr.Src1 + 2), RAX); // cap = 0
          end;

        // === Map/Set operations (v0.5.0) ===
        // Simple array-based implementation (linear search)
        // Map structure: [len:8][cap:8][entries:16*cap]
        // Entry: [key:8][value:8]
        irMapNew, irSetNew:
          begin
            // Allocate: 16 (header) + 16*8 (8 entries initial)
            // mmap(addr=0, len=144, prot=RW, flags=MAP_ANON|MAP_PRIVATE, fd=-1, off=0)
            WriteMovRegImm64(FCode, RAX, 9); // sys_mmap
            WriteMovRegImm64(FCode, RDI, 0);
            WriteMovRegImm64(FCode, RSI, 144); // 16 + 16*8
            WriteMovRegImm64(FCode, RDX, 3); // PROT_READ | PROT_WRITE
            WriteMovRegImm64(FCode, R10, 34); // MAP_ANON | MAP_PRIVATE
            WriteMovRegImm64(FCode, R8, High(UInt64)); // fd = -1
            WriteMovRegImm64(FCode, R9, 0);
            WriteSyscall(FCode);
            // Store pointer in dest
            WriteMovMemReg(FCode, RBP, SlotOffset(localCnt + instr.Dest), RAX);
            // Initialize: len=0, cap=8 at [rax+0] and [rax+8]
            // mov qword [rax], 0
        ...
        irMapSet:
          begin
            ...
            // First search for existing key, update if found, else append
            ...
            // r8 = 0 (counter): xor r8d, r8d
            EmitU8(FCode, $45); EmitU8(FCode, $31); EmitU8(FCode, $C0);
            // r9 = rdi + 16 (first entry): lea r9, [rdi+16]
            EmitU8(FCode, $4C); EmitU8(FCode, $8D); EmitU8(FCode, $4F); EmitU8(FCode, $10);
            
            // Search loop
            loopStartPos := FCode.Size;
            // cmp r8, rcx (counter vs len)
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $C8);
            // jge notFound (will append)
            jgePos := FCode.Size;
            EmitU8(FCode, $7D); EmitU8(FCode, $00); // placeholder
            // r10 = [r9] (entry key)
            EmitU8(FCode, $4D); EmitU8(FCode, $8B); EmitU8(FCode, $11);
            // cmp r10, rsi (entry key vs search key)
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $F2);
            // jne next
            jneLoopPos := FCode.Size;
            EmitU8(FCode, $75); EmitU8(FCode, $00); // placeholder
            // Found: update value at [r9+8] and jump to done
            // mov [r9+8], rdx
            EmitU8(FCode, $49); EmitU8(FCode, $89); EmitU8(FCode, $51); EmitU8(FCode, $08);
            // jmp done
            jmpDonePos := FCode.Size;
            EmitU8(FCode, $EB); EmitU8(FCode, $00); // placeholder
            // Next: add r9, 16; inc r8; jmp loop
            nextLabelPos := FCode.Size;
            EmitU8(FCode, $49); EmitU8(FCode, $83); EmitU8(FCode, $C1); EmitU8(FCode, $10); // add r9, 16
            EmitU8(FCode, $49); EmitU8(FCode, $FF); EmitU8(FCode, $C0); // inc r8
            // jmp loop
            EmitU8(FCode, $EB);
            EmitU8(FCode, Byte(Int8(loopStartPos - (FCode.Size + 1))));
            
            // notFound: append new entry
            notFoundPos := FCode.Size;
            // r9 = rdi + 16 + rcx*16 (end of entries)
            // lea r9, [rdi + rcx*1 + 16] won't work, need manual: r9 = rdi; r10 = rcx; shl r10, 4; add r9, r10; add r9, 16
            WriteMovRegReg(FCode, R9, RDI);
            WriteMovRegReg(FCode, R10, RCX);
            EmitU8(FCode, $49); EmitU8(FCode, $C1); EmitU8(FCode, $E2); EmitU8(FCode, $04); // shl r10, 4
            EmitU8(FCode, $4D); EmitU8(FCode, $01); EmitU8(FCode, $D1); // add r9, r10
            EmitU8(FCode, $49); EmitU8(FCode, $83); EmitU8(FCode, $C1); EmitU8(FCode, $10); // add r9, 16
            // Store key: mov [r9], rsi
            EmitU8(FCode, $49); EmitU8(FCode, $89); EmitU8(FCode, $31);
            // Store value: mov [r9+8], rdx
            EmitU8(FCode, $49); EmitU8(FCode, $89); EmitU8(FCode, $51); EmitU8(FCode, $08);
            // Increment len: inc qword [rdi]
        ...
        irSetAdd:
          begin
            // set_add(set, value) - same as map_set but value=0
            WriteMovRegMem(FCode, RDI, RBP, SlotOffset(localCnt + instr.Src1));
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(localCnt + instr.Src2));
            // mov rcx, [rdi]
            EmitU8(FCode, $48); EmitU8(FCode, $8B); EmitU8(FCode, $0F);
        ...
        irMapGet:
          begin
            ...
            WriteMovRegMem(FCode, RDI, RBP, SlotOffset(localCnt + instr.Src1)); // map
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(localCnt + instr.Src2)); // key
            // rcx = len: mov rcx, [rdi]
            EmitU8(FCode, $48); EmitU8(FCode, $8B); EmitU8(FCode, $0F);
            // r8 = 0 (counter): xor r8d, r8d
            EmitU8(FCode, $45); EmitU8(FCode, $31); EmitU8(FCode, $C0);
            // rax = 0 (default): xor eax, eax
            ...
            // Loop start
            loopStartPos := FCode.Size;
            // cmp r8, rcx  (REX.W+REX.B, CMP r/m64,r64, ModRM: reg=rcx, r/m=r8)
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $C8);
            // jge done (placeholder)
            jgePos := FCode.Size;
            EmitU8(FCode, $7D); EmitU8(FCode, $00); // short jge
            // r10 = [r9] (entry key)
            EmitU8(FCode, $4D); EmitU8(FCode, $8B); EmitU8(FCode, $11);
            // cmp r10, rsi  (REX.W+REX.B, CMP r/m64,r64, ModRM: reg=rsi, r/m=r10)
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $F2);
            // jne next (placeholder)
            jneLoopPos := FCode.Size;
            EmitU8(FCode, $75); EmitU8(FCode, $00); // short jne
            // Found: rax = [r9+8]
            EmitU8(FCode, $49); EmitU8(FCode, $8B); EmitU8(FCode, $41); EmitU8(FCode, $08);
            // jmp done (placeholder)
            jmpDonePos := FCode.Size;
            EmitU8(FCode, $EB); EmitU8(FCode, $00); // short jmp
            // Next: add r9, 16; inc r8; jmp loop
            nextLabelPos := FCode.Size;
            EmitU8(FCode, $49); EmitU8(FCode, $83); EmitU8(FCode, $C1); EmitU8(FCode, $10); // add r9, 16
            EmitU8(FCode, $49); EmitU8(FCode, $FF); EmitU8(FCode, $C0); // inc r8
        ...
        irMapContains, irSetContains:
          begin
            // contains(map, key) -> bool (similar to MapGet but returns 0/1)
            WriteMovRegMem(FCode, RDI, RBP, SlotOffset(localCnt + instr.Src1));
            WriteMovRegMem(FCode, RSI, RBP, SlotOffset(localCnt + instr.Src2));
            EmitU8(FCode, $48); EmitU8(FCode, $8B); EmitU8(FCode, $0F); // mov rcx, [rdi]
            EmitU8(FCode, $45); EmitU8(FCode, $31); EmitU8(FCode, $C0); // xor r8d, r8d
            EmitU8(FCode, $31); EmitU8(FCode, $C0); // xor eax, eax (result = false)
            EmitU8(FCode, $4C); EmitU8(FCode, $8D); EmitU8(FCode, $4F); EmitU8(FCode, $10); // lea r9, [rdi+16]
            
            loopStartPos := FCode.Size;
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $C8); // cmp r8, rcx (fixed encoding)
            jgePos := FCode.Size;
            EmitU8(FCode, $7D); EmitU8(FCode, $00);
            EmitU8(FCode, $4D); EmitU8(FCode, $8B); EmitU8(FCode, $11); // mov r10, [r9]
            EmitU8(FCode, $49); EmitU8(FCode, $39); EmitU8(FCode, $F2); // cmp r10, rsi (fixed encoding)
            jneLoopPos := FCode.Size;
        ...
        irMapLen, irSetLen:
          begin
            // len(map) -> int64: return [map+0]
            WriteMovRegMem(FCode, RDI, RBP, SlotOffset(localCnt + instr.Src1));
            EmitU8(FCode, $48); EmitU8(FCode, $8B); EmitU8(FCode, $07); // mov rax, [rdi]
            WriteMovMemReg(FCode, RBP, SlotOffset(localCnt + instr.Dest), RAX);
          end;

        irMapRemove, irSetRemove:
          begin
  ...
  begin
    leaPos := FVMTLeaPositions[i].CodePos;
    vmtLabelIdx := FVMTLeaPositions[i].VMTIndex;
    codeVA := 4096;
    instrVA := codeVA + leaPos + 7;
    // VMT position is stored in FVMTLabels[vmtLabelIdx].Pos
    dataVA := 4096 + ((UInt64(FCode.Size) + 4095) and not UInt64(4095)) + FVMTLabels[vmtLabelIdx].Pos;
    disp32 := Int64(dataVA) - Int64(instrVA);
    FCode.PatchU32LE(leaPos + 3, Cardinal(disp32));
  end;
  ...
  begin
    cd := module.ClassDecls[i];
    // Only patch if class has virtual methods
    if Length(cd.VirtualMethods) > 0 then
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // Code base is 0x401000 (4096 + 0x400000)
            FData.PatchU64LE(vmtDataPos + (j * 8), UInt64($401000 + methodAddr));
          end
          else
          begin
